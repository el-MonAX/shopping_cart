RSpec.shared_examples 'address_validations' do
  ...
  it { expect(subject).to allow_value('Jane').for(:first_name) }
  it { expect(subject).to allow_value('Doe').for(:last_name) }
  it { expect(subject).to allow_value('NY').for(:city) }
  it { expect(subject).to allow_value('Street, 7').for(:address) }
